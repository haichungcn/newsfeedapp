...
async function fetchNews(country, category, topic) {
    let url = `https://newsapi.org/v2/top-headlines?country=${country == null ? 'us' : country}${category == null ? '' : `&category=${category}`}${topic == null ? '' : `&topic=${topic}`}&apiKey=c8920a6401f9458eae1a7af498f324c4&page=${pageNum}`
    console.log(url);
    let response = await fetch(url);
    let data = await response.json();
    data.totalResults > 0 ? newsArticles = newsArticles.concat(data.articles) : console.log('There is no news fetched');
...
const filterSources = (articles) => {
    articles.map(a => {
        if(!sourceNames.includes(a.source.name)){
            sourceNames.push(a.source.name)
        };
    })
...
const displaySources = (arr) => {
    const html = arr.map((sourceName, i) => {
        return `
        <li class="list-group-item list-group-item-action pl-1 pl-sm-5"><input type="checkbox" class="form-check-input" id="${sourceName}" onchange="renderNewsBySource(${i})"><label for="${sourceName}">${sourceName.split('.')[0]}</label></li>
        `
    }).join('');
...
const renderNewsBySource = (idx) => {
    let checkbox;
    checkbox = document.getElementById(sourceNames[idx]);
    if (checkbox.checked){
        sortBy = "source";
        selectedSourceNames.push(sourceNames[idx]);
        checkbox.parentElement.classList.add("active");
        // console.log('checkbox has been checked, running renderNews now, selectedSourceNames is: ', selectedSourceNames);
        renderNews(newsArticles);
    } else {
        selectedSourceNames.map((val,i) => sourceNames[idx] == val && selectedSourceNames.splice(i,1));
